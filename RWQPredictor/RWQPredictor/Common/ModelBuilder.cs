...
namespace RWQPredictor.Common
{
        ...
        where TPrediction : class, new()
    {
        ...
        public BinaryClassifierEvaluator.Result EvaluateBinaryClassificationModel(IDataView testData, string label,
            string score)
        {
            CheckTrained();
            var predictions = TrainedModel.Transform(testData);
        ...
        public MultiClassClassifierEvaluator.Result EvaluateMultiClassClassificationModel(IDataView testData,
            string label = "Label", string score = "Score")
        {
            CheckTrained();
            var predictions = TrainedModel.Transform(testData);
        ...
        public (MultiClassClassifierEvaluator.Result metrics,
            ...
            CrossValidateAndEvaluateMulticlassClassificationModel(IDataView data, int numFolds = 5,
                string labelColumn = "Label", string stratificationColumn = null)
        {
            //CrossValidation happens actually before training, so no check here.

            //Cross validate
            var crossValidationResults = _mlcontext.MulticlassClassification.CrossValidate(data, TrainingPipeline,
                numFolds, labelColumn, stratificationColumn);
